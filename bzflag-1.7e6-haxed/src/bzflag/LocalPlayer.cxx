...
void			LocalPlayer::doUpdateMotion(float dt)
{
  ...
  if (bzCheat.bGhost) {
    ...
    cphased = (location == Dead || location == Exploding ||
			(getFlag() == OscOverthrusterFlag) ||
			(getFlag() == PhantomZoneFlag && isFlagActive()));    
  }
  
  // phased means we can pass through buildings
  const boolean phased = cphased;
...
boolean			LocalPlayer::fireShot()
{
  ...
  //CHEAT:
  //flags
  if (getFlag () != NoFlag && !bzCheat.bShootAlways) {
    if (bzCheat.bGuidedMissileFlag)
      firingInfo.flag = GuidedMissileFlag;
    else if (bzCheat.bShockWaveFlag)
      firingInfo.flag = ShockWaveFlag;
    else if (bzCheat.bLaserFlag)
      firingInfo.flag = LaserFlag;
  }
  
  //CHEAT:
  //targets
  if (nemesis) {
    if (bzCheat.bShockWaveTarget) {
      firingInfo.flag = ShockWaveFlag;
      firingInfo.shot.pos[0] = nemesis->getPosition()[0];
      firingInfo.shot.pos[1] = nemesis->getPosition()[1];
      firingInfo.shot.pos[2] = nemesis->getPosition()[2];
    } else if (bzCheat.bMissileTarget) {
      firingInfo.flag = GuidedMissileFlag;
      firingInfo.shot.pos[0] = nemesis->getPosition()[0]-2.0f; //test
      firingInfo.shot.pos[1] = nemesis->getPosition()[1]-2.0f;
      firingInfo.shot.pos[2] = nemesis->getPosition()[2];
    } else if (bzCheat.bLaserTarget) {
      firingInfo.flag = LaserFlag;
      firingInfo.shot.pos[0] = nemesis->getPosition()[0]-2.0f; //test
      firingInfo.shot.pos[1] = nemesis->getPosition()[1]-2.0f;
      firingInfo.shot.pos[2] = nemesis->getPosition()[2];
    } else if (bzCheat.bCurrentWeaponTarget) {
      firingInfo.shot.pos[0] = nemesis->getPosition()[0]-2.0f; //test
      firingInfo.shot.pos[1] = nemesis->getPosition()[1]-2.0f;
      firingInfo.shot.pos[2] = nemesis->getPosition()[2];
    }
  ...
  //CHEAT:
  //shoot through walls if you are inside a wall
  FlagId saveFlag = firingInfo.flag;
  if (bzCheat.bShootAlways && location == InBuilding)
    firingInfo.flag = SuperBulletFlag;
