...
{
  //maybe this should go into bzcheat.cxx
  //CHEAT: CODE ACTIVATION (menu or direct keys)
  //added code in XDisplay.cxx to handle scroll mouse
  if (bzCheat.bDisplayMenu && pressed) {
    if (key.button == BzfKeyEvent::MiddleMouse && key.ascii == 1 /*scrolled up*/) {
      bzCheat.DecreaseMenuPos ();
    } else if (key.button == BzfKeyEvent::MiddleMouse && key.ascii == 2 /*scrolled down*/) {
      bzCheat.IncreaseMenuPos ();
    } else if (key.button == BzfKeyEvent::LeftMouse) {
      bzCheat.MenuAction1 ();
    } else if (key.button == BzfKeyEvent::RightMouse) {
      bzCheat.MenuAction2 ();
    }
    
    if (key.ascii == 27 || key.ascii == 'e') {
      bzCheat.bDisplayMenu = false;
    }
    
    return True; //don't let the game process the keys
  }
  
  if (key.ascii == 'e' || key.button == BzfKeyEvent::MiddleMouse && key.ascii == 2) {
    if (pressed)
      bzCheat.bDisplayMenu = !bzCheat.bDisplayMenu;
  }
  //
...
static void		checkEnvironment()
{
  ...
  //CHEAT: identify flags (does what the last else if code does above (i copied it))
  if (bzCheat.bIdentify) {
    const float* tpos = myTank->getPosition();
    BzfString message("Closest Flag: ");
    float minDist = IdentityRange * IdentityRange;
